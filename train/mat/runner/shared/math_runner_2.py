...
class MathRunner:
    ...
    def run(self):
        ...
        if self.prm_type != 'AI':
            ...
            while self.total_num_steps < self.num_env_steps:
                ...
                for step in range(
                        int(self.episode_length * 1.5)):  # for more chances of collecting trajectories, since episode_length = max_step_envs
                    # Sample actions
                    values, actions, action_tokens, log_probs = self.collect(
                        np.concatenate(last_obs))  # change this for trajectory based
                    # Obs reward and next obs
                    last_obs, rewards, dones, infos = self.envs.extended_step(values, actions, action_tokens,
                                                                              log_probs)  # annoying there here -- if it's done it's going to automaitcally reset
                    ...
                    for i in range(self.n_rollout_threads):
                        if dones[
                            i, 0]:  # dones, and any other thing returned from self.envs.step is going to be in the shape num_rollouts, num_agents
                            # episodic_returns.append(rewards[i, 0])
                            reasoning_chain, obs, action, action_token, log_prob, value, problem = infos[i][
                                'trajectory']  # remember these are ALREADY COPIED NP ARRAYS of shape (max_steps (episode_length), n_agents (1))
                    ...
                    if len(done_reasonings) > 0:
                        ...
                        rewards = self.prm.get_reasoning_traj_reward_avg(done_reasonings,
                                                                     problems, 5)  # (num_trajectories, max_length) numpy array
                        for ind in range(len(done_reasonings)):
                            ...
                            self.insert_trajectory((
                                                   traj_obs[ind], reward, values[ind], actions[ind], action_tokens[ind],
                                                   log_probs[ind]))  # maintain buffer size by FIFO RULE
    ...
    def insert(self, data):
        ...
        masks = np.ones((self.n_rollout_threads, self.num_agents),
                        dtype=np.float32)  # create mask of size (# of envs, # of agents (1))
